async function getPosts() {
   return await fetch('http://localhost:3000/posts')
    .then((response) => response.json())            //converting into a readable format
    .then((data) => data);                          //returning converted data
}
...
document.addEventListener('DOMContentLoaded', async function() { // Page loading
   addPosts();
   addCallbackRequests();
   addEmails();
    ...
    addPostBtn.addEventListener('click', () => createPostBtn)
})
...
async function addCallbackRequests() {
    ...
    requests.forEach((request) => {
        ...
        `;
        articles.insertAdjacentHTML('beforeend', requestHTML);
    })
}
...
async function addEmails() {
    ...
    emails.forEach((email) => {
        ...
        `;
        articles.insertAdjacentHTML('beforeend', emailHTML);
    })
}
